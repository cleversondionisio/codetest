<x-app-layout>
  <x-card class="p-10 max-w-lg mx-auto mt-24">
    ...
    <form method="POST" action="/dashboard/attendees/edit/{{$attendee->id}}">
      @csrf
      @method('PUT')
      ...
      <div class="mb-6">
        <label for="approved" class="inline-block text-lg mb-2">Approve</label>
          ...
          placeholder="Example: Remote, Boston MA, etc" value="">
            <option value=1 {{$attendee->approved == 1 ? "selected=selected" : ''}}>True</option>
            <option value=0 {{$attendee->approved == 0 ? "selected=selected" : ''}}>False</option>
          </select>
      ...
      <div class="mb-6">
        ...
        <input type="text" class="border border-gray-200 rounded p-2 w-full" name="email" value="{{$attendee->email}}" />
